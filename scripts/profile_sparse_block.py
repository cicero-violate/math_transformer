...
def main() -> None:
    ...
    p.add_argument("--topology-mode", default="middle_preserving_topk", choices=["union", "scored_topk", "middle_preserving_topk", "learned_topology"])
    p.add_argument("--fixed-k", type=int, default=16)
    p.add_argument("--learned-scorer-checkpoint", default=None)
    p.add_argument("--learned-k", type=int, default=8)
    ...
    block = MathRoutedTransformerBlock(
        ...
        topology_mode=("middle_preserving_topk" if args.topology_mode == "learned_topology" else args.topology_mode),
        fixed_k=args.fixed_k,
        middle_bridge_width=args.middle_bridge_width,
        triton_block_d=args.triton_block_d,
        triton_block_k=args.triton_block_k,
    ).to(dev)
    if args.topology_mode == "learned_topology":
        if not args.learned_scorer_checkpoint:
            raise ValueError("--learned-scorer-checkpoint is required for learned_topology")
        block.topology = LearnedTopologyBuilder(
            args.learned_scorer_checkpoint,
        ...
        block.max_neighbors = args.learned_k
    block.eval()
    ...
    print(
        ...
        f"topology_mode={args.topology_mode} fixed_k={args.fixed_k} learned_k={args.learned_k} max_neighbors={block.max_neighbors}"
    )
