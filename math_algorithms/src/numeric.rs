...
pub fn simplex_two_variable(
    c: [f64; 2],
    constraints: &[([f64; 2], f64)],
) -> Option<([f64; 2], f64)> {
    ...
    points
        ...
        .filter(|p| {
            constraints
                .iter()
                .all(|&([a, b], rhs)| a * p[0] + b * p[1] <= rhs + 1e-9)
        })
...
mod tests {
    ...
    fn numeric_kernels_work() {
        assert_eq!(
            matrix_multiply(&[vec![1.0, 2.0]], &[vec![3.0], vec![4.0]]),
            vec![vec![11.0]]
        );
        let x =
            gaussian_elimination(vec![vec![2.0, 1.0], vec![1.0, -1.0]], vec![5.0, 1.0]).unwrap();
        assert!((x[0] - 2.0).abs() < 1e-9);
        assert!((x[1] - 1.0).abs() < 1e-9);
        let (_, value) =
            simplex_two_variable([3.0, 2.0], &[([1.0, 1.0], 4.0), ([1.0, 0.0], 2.0)]).unwrap();
